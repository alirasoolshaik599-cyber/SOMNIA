...
import MildStepCard from "@/components/induction/MildStepCard";
import MildPhraseCard from "@/components/induction/MildPhraseCard";
...
import { mildOverview, mildSteps, mildCommonMistakes, mildTips } from "@/lib/mildContent";
...
export default function MildTrainingPage() {
  const { phrase, hasLoaded: phraseLoaded, setPhrase } = useMildPhrase();
  const {
    entries,
    hasLoaded: logLoaded,
    saveTodayEntry,
    deleteEntry,
    getTodayEntry,
  } = usePracticeLog(MILD_PRACTICE_LOG_KEY, "mild-practice");
  const {
    hasLoaded: sessionsLoaded,
    practicedDates,
    lastEntry: lastGuidedEntry,
  } = useGuidedSessionLog(MILD_GUIDED_SESSIONS_KEY, "mild-session");
  
  const { requireAuth } = useAuthContext();
  const hasLoaded = phraseLoaded && logLoaded && sessionsLoaded;
    ...
    <main className="relative min-h-dvh px-4 pb-16 pt-10 sm:px-6 sm:pb-20 sm:pt-14">
      ...
      <div className="relative z-10 mx-auto w-full max-w-3xl">
        ...
        <div className="mt-10 sm:mt-14">
          <PageHeader
            title="MILD Training"
            description="A calm, guided path through Mnemonic Induction of Lucid Dreams."
          />
            ...
            <>
              <div className="mt-12 space-y-3 rounded-3xl border border-white/10 bg-black/25 p-5 backdrop-blur-md sm:p-6">
                <div>
                  ...
                  <p className="mt-1 text-sm leading-6 text-slate-300">{mildOverview.whatItIs}</p>
                </div>
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">
                    Why it works
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm leading-6 text-slate-300">{mildOverview.bestFor}</p>
                </div>
              </div>

              <div className="mt-14">
                <SectionHeading>Guided Flow</SectionHeading>
                <div className="space-y-4">
                  {mildSteps.map((step, index) => (
                    <MildStepCard key={step.id} step={step} index={index} />
                  ))}
                </div>
              </div>

              <div className="mt-14">
                <SectionHeading>Your MILD Phrase</SectionHeading>
                <MildPhraseCard
                  currentText={phrase?.text ?? ""}
                  onSave={(text) => requireAuth(() => setPhrase(text))}
                />
              </div>
                ...
                <div className="space-y-5">
                  <div className="rounded-3xl border border-[#5B6EFF]/20 bg-[#5B6EFF]/10 p-5 text-center backdrop-blur-md sm:p-8">
                    <p className="text-sm leading-6 text-slate-300">
                      Ready to practice? Walk through tonight&apos;s session one calm step at a
                      time.
                    </p>
                    <div className="mt-5">
                      <PillLink href="/induction/mild/practice" variant="primary">
                        Start Tonight&apos;s Guided Session
                      </PillLink>
                    </div>
                  </div>
                  ...
                  <LastPracticeSummary
                    lastEntry={lastGuidedEntry}
                    extraFields={ 
                      phrase?.text ? [{ label: "Phrase", value: `"${phrase.text}"` }] : [] 
                    }    
                  />
                </div>
              </div>
              ...
              <div className="mt-14">
                ...
                <div className="space-y-6 rounded-3xl border border-white/10 bg-black/35 p-5 backdrop-blur-md shadow-[0_0_25px_rgba(0,0,0,0.25)] sm:p-8">
                  ...
                  <TextArea
                    ...
                    placeholder="How did the practice feel tonight?"
              ...
              <div className="mt-14 grid grid-cols-1 gap-5 sm:grid-cols-2">
                <div className="rounded-3xl border border-white/10 bg-black/25 p-5 backdrop-blur-md sm:p-6">
                  ...
                  <ul className="mt-4 space-y-3">
                    {mildCommonMistakes.map((mistake, index) => (
