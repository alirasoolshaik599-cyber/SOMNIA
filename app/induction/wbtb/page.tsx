...
import { wbtbOverview, wbtbPreparationTips, wbtbTips, wbtbCommonMistakes } from "@/lib/wbtbContent";
...
export default function WbtbTrainingPage() {
  const {
    entries,
    hasLoaded: logLoaded,
    saveTodayEntry,
    deleteEntry,
    getTodayEntry,
  } = usePracticeLog(WBTB_PRACTICE_LOG_KEY, "wbtb-practice");
  const {
    hasLoaded: sessionsLoaded,
    practicedDates,
    lastEntry: lastGuidedEntry,
  } = useGuidedSessionLog(WBTB_GUIDED_SESSIONS_KEY, "wbtb-session");
      ...
      <div className="relative z-10 mx-auto w-full max-w-3xl">
        ...
        <div className="mt-10 sm:mt-14">
          <PageHeader
            ...
            description="A calm, well-timed pause in the middle of the night — SOMNIA's second guided technique."
          />
            ...
            <>
              <div className="mt-12 space-y-3 rounded-3xl border border-white/10 bg-black/25 p-5 backdrop-blur-md sm:p-6">
                <div>
                  ...
                  <p className="mt-1 text-sm leading-6 text-slate-300">{wbtbOverview.whatItIs}</p>
                </div>
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">
                    Why it works
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm leading-6 text-slate-300">{wbtbOverview.bestFor}</p>
                </div>
              </div>
              ...
              <div className="mt-14">
                ...
                <div className="rounded-3xl border border-white/10 bg-black/25 p-5 backdrop-blur-md sm:p-6">
                  <ul className="space-y-3">
                    {wbtbPreparationTips.map((tip, index) => (
                ...
                <div className="space-y-5">
                  <div className="rounded-3xl border border-[#5B6EFF]/20 bg-[#5B6EFF]/10 p-5 text-center backdrop-blur-md sm:p-8">
                    <p className="text-sm leading-6 text-slate-300">
                      Ready to practice? Walk through the WBTB process one calm step at a time.
                    </p>
                    <div className="mt-5">
                      <PillLink href="/induction/wbtb/practice" variant="primary">
                        Start Guided WBTB Session
                      </PillLink>
                    </div>
                  </div>
                  ...
                  <LastPracticeSummary
                    lastEntry={lastGuidedEntry}
                    emptyMessage="No guided WBTB sessions yet. Your first one will appear here."
                  />
                </div>
              </div>
              ...
              <div className="mt-14 grid grid-cols-1 gap-5 sm:grid-cols-2">
                <div className="rounded-3xl border border-white/10 bg-black/25 p-5 backdrop-blur-md sm:p-6">
                  ...
                  <ul className="mt-4 space-y-3">
                    {wbtbCommonMistakes.map((mistake, index) => (
                      <li key={index} className="flex gap-2 text-sm leading-6 text-slate-300">
                        <span className="shrink-0 text-[#a9b3ff]" aria-hidden="true">
                          •
                        </span>
