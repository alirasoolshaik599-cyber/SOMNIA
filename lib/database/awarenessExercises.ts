...
export async function getAwarenessExerciseCompletionsFromDatabase(): Promise<ExerciseCompletion[]> {
  const user = await getCurrentUser();

  if (!user) return [];

  const { data, error } = await supabase
    .from("awareness_exercise_completions")
...
export async function markAwarenessExerciseDoneInDatabase(exerciseId: string): Promise<void> {
  const user = await getCurrentUser();
  ...
  const { error } = await supabase.from("awareness_exercise_completions").upsert(
    {
      user_id: user.id,
